...
import { Country, getCountryConfig, ScrapedArticle, Website } from "@/data/countryConfig";
...
const Index = () => {
  ...
  const getFilteredArticles = () => {
    ...
    if (selectedWebsite !== 'all') {
      filtered = filtered.filter(a => a.source === selectedWebsite);
    }

    if (selectedDate) {
      filtered = filtered.filter(a => a.date === formatDate(selectedDate));
    }
  ...
  const sortedArticles = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const aUrgent = a.tags.includes('urgent');
      const bUrgent = b.tags.includes('urgent');
  ...
  }, [articles, selectedWebsite, selectedDate, searchTerm, documentType, subject, interest]);
        ...
        <div className="grid gap-6">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="ml-2">
              {labels.articleCount(sortedArticles.length)}
            </Badge>
          </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {article.tags.map(tag => (
                          <ArticleTagBadge key={tag} tag={tag} labels={labels} />
                        ))}
                      </div>
